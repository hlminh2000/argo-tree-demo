...
class Spider extends SpiderBase {
  ...
  renderNodes() {
    ...
    return nodes.toKeyedSeq().map(node => {
      ...
      if (nodeTransform) {
        ...
        groupTransform = new Transform().translate(
          projectedNode[0],
          projectedNode[1]
        );
      }
  ...
  passProjection(child) {
    ...
    const cloneProps = {
      data: props.data,
      projection:
        props.projection || this.props.linkProjection || this.props.projection,
  ...
  render() {
    ...
    const groupTransform = transformFunction.translate(
      left + offsetLeft,
      top + offsetTop
    );
    // node width
    return (
      <div ref="cursorHelper">
